...
namespace Vetapp.Engine.BusinessAccessLayer
{
    ...
    public class BusContentType
    {
        ...
        {
            return (Get(0, new DateTime(), new DateTime(), null, null, null, 0, false));
        }
        ...
        {
            return (Get(lContentTypeID, new DateTime(), new DateTime(), null, null, null, 0, false));
        }
        ...
        {
            return (Get(o.ContentTypeID, o.DateCreated, o.DateCreated, o.Code, o.Description, o.VisibleCode, o.MaxRating, o.HasSides));
        }
        ...
        {
            return (Get(o.ContentTypeID, o.BeginDateCreated, o.EndDateCreated, o.Code, o.Description, o.VisibleCode, o.MaxRating, o.HasSides));
        }
        ...
        ///     <retvalue>ArrayList containing ContentType object</retvalue>
        /// </summary>
        public ArrayList Get(long pLngContentTypeID, DateTime pDtBeginDateCreated, DateTime pDtEndDateCreated, string pStrCode, string pStrDescription, string pStrVisibleCode, long pLngMaxRating, bool? pBolHasSides)
        {
        ...
        public bool IsValid(ContentType pRefContentType)
        {
            ...
            isValidTmp = IsValidMaxRating(pRefContentType.MaxRating);
            if (!isValidTmp)
            {
                isValid = false;
            }
            isValidTmp = IsValidHasSides(pRefContentType.HasSides);
            if (!isValidTmp && pRefContentType.HasSides != null)
            {
                isValid = false;
            }
        ...
        ///     <retvalue>true if object has a valid entry, false otherwise</retvalue>
        /// </summary>
        public bool IsValidMaxRating(long pLngData)
        {
            bool isValid = true;

            // do some validation
            isValid = (new Regex(REGEXP_ISVALID_MAX_RATING)).IsMatch(pLngData.ToString());
            if (!isValid)
            {
                Column clm = null;
                clm = new Column();
                clm.ColumnName = ContentType.DB_FIELD_MAX_RATING;
                clm.HasError = true;
                _arrlstColumnErrors.Add(clm);
                _hasInvalid = true;
            }
        ...
        ///     <retvalue>true if object has a valid entry, false otherwise</retvalue>
        /// </summary>
        public bool IsValidHasSides(bool? pBolData)
        {
            bool isValid = true;

            // do some validation
            isValid = (new Regex(REGEXP_ISVALID_HAS_SIDES)).IsMatch(pBolData.ToString());
            if (!isValid)
            {
                Column clm = null;
                clm = new Column();
                clm.ColumnName = ContentType.DB_FIELD_HAS_SIDES;
                clm.HasError = true;
                _arrlstColumnErrors.Add(clm);
                _hasInvalid = true;
            }
