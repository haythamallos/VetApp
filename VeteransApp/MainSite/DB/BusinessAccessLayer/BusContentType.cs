...
namespace Vetapp.Engine.BusinessAccessLayer
{
    ...
    public class BusContentType
    {
        ...
        public bool IsValid(ContentType pRefContentType)
        {
            ...
            isValidTmp = IsValidProductRefDescription(pRefContentType.ProductRefDescription);
            if (!isValidTmp && pRefContentType.ProductRefDescription != null)
            {
                isValid = false;
            }
        ...
        public bool IsValidProductRefName(string pStrData)
        {
            ...
            // do some validation
            isValid = !(new Regex(REGEXP_ISVALID_PRODUCT_REF_NAME)).IsMatch(pStrData);
            if (!isValid)
            {
                Column clm = null;
                clm = new Column();
                clm.ColumnName = ContentType.DB_FIELD_PRODUCT_REF_NAME;
                clm.HasError = true;
                _arrlstColumnErrors.Add(clm);
                _hasInvalid = true;
            }
        ...
        ///     <retvalue>true if object has a valid entry, false otherwise</retvalue>
        /// </summary>
        public bool IsValidProductRefDescription(string pStrData)
        {
            bool isValid = true;

            // do some validation
            isValid = !(new Regex(REGEXP_ISVALID_PRODUCT_REF_DESCRIPTION)).IsMatch(pStrData);
            if (!isValid)
            {
                Column clm = null;
                clm = new Column();
                clm.ColumnName = ContentType.DB_FIELD_PRODUCT_REF_DESCRIPTION;
                clm.HasError = true;
                _arrlstColumnErrors.Add(clm);
                _hasInvalid = true;
            }
        ...
        ///     <retvalue>true if object has a valid entry, false otherwise</retvalue>
        /// </summary>
        public bool IsValidNumberOfPages(long pLngData)
        {
            bool isValid = true;

            // do some validation
            isValid = (new Regex(REGEXP_ISVALID_NUMBER_OF_PAGES)).IsMatch(pLngData.ToString());
            if (!isValid)
            {
                Column clm = null;
                clm = new Column();
